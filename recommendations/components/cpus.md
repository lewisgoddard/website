...
|------------|------------|-------|-------|----------|
| Intel 200 | Core Ultra 9 285K | 24 / 8+16 | ~£490 | Ultra |
|  | Core Ultra 7 265K | 20 / 8+12 | ~£360 | High |
|  | Core Ultra 5 245K | 14 / 6+8 | ~£270 | Medium |
| Intel 14 | i9 14900K | 32 / 8+16 | ~£370 | Ultra |
|  | i7 14700K | 28 / 8+12 | ~£325 | High |
|  | **i5 14600K** | 20 / 6+8 | **~£240** | Medium |
|  | i5 14400 | 16 / 6+4 | ~£160 | Medium |
|  | i3 14100 | 8 / 4+0 | ~£110 | Basic |
...
| AMD 9000 | R9 9950X | 32 / 16 | ~£500 | Ultra |
|  | R9 9900X | 24 / 12 | ~£360 | High |
|  | **R7 9700X** | 16 / 8 | **~£280** | Medium |
|  | **R5 9600X** | 12 / 6 | **~£205** | Medium |
...
| Generation | Family SKU | Cores | Price | Best for |
|------------|------------|-------|-------|----------|
| Intel 200 | Core Ultra 9 285K | 24 / 8+16 | ~£490 | High |
|  | Core Ultra 7 265KF | 20 / 8+12 | ~£345 | High |
|  | **Core Ultra 5 245KF** | 14 / 6+8 | **~£250** | Medium |
| Intel 14 | i9 14900KF | 32 / 8+16 | ~£360 | High |
|  | i7 14700KF | 28 / 8+12 | ~£315 | High |
|  | **i5 14600KF** | 20 / 6+8 | **~£225** | Medium |
|  | **i5 14400F** | 16 / 6+4 | **~£150** | Medium |
|  | **i3 14100F** | 8 / 4+0 | **~£90** | Basic |
...
| AMD 9000 | **R7 9800X3D** | 16 / 8 | **~£410** | Ultra |
|  | R9 9950X | 32 / 16 | ~£500 | High |
|  | R9 9900X | 24 / 12 | ~£360 | High |
|  | **R7 9700X** | 16 / 8 | **~£280** | Medium |
|  | **R5 9600X** | 12 / 6 | **~£205** | Medium |
| AMD 7000 | **R7 7800X3D** | 16 / 8 | **~£310** | Ultra |
|  | R9 7950X | 32 / 16 | ~£530 | High |
|  | R9 7900X | 24 / 12 | ~£400 | High |
|  | R7 7700X | 16 / 8 | ~£300 | Medium |
|  | **R5 7600X** | 12 / 6 | **~£230** | Medium |
| AMD 5000 | **R7 5800X3D** | 16 / 8 | **~£160** | High |
|  | **R7 5800X** | 16 / 8 | **~£200** | Medium |
...
- Intel 200 (Arrow Lake) moves to a new LGA1851 platform with DDR5-only support and a tile-based chip design. Hyperthreading has been dropped from performance cores, so thread counts look lower than Intel 14 despite similar core layouts. Gaming performance was initially disappointing at launch but improved substantially with firmware and driver updates. Intel Arc graphics are now on-die for non-F SKUs.
- Intel 14 is essentially Intel 13 at slightly higher clocks, on the same LGA1700 platform with DDR4 and DDR5 support. The main exception is the 14700K, which gained four additional efficiency cores. If you already have Intel 13, there is no reason to upgrade.
...
I have heard that generational improvement of Intel silicon is largely down to pushing more power for diminishing returns. While this adage may have been true for a time (14nm was almost exclusively in use from 5th gen / 2014 - 10th gen 2019) it no longer holds true. 12th gen Intel (Alder Lake) brought with them a new 10nm process and a move to a new naming scheme for such processes in line with AMDs naming, thus "Intel 7". It also brought a bit.LITTLE style core arrangement, with single-thread efficiency cores coupled with the existing dual-thread performance cores and continued DDR4 support throwing a substantial wrench in AMDs move towards price parity as a true competitor. 13th gen Intel (Raptor Lake) brought mild improvements to IPC and expanded the number of efficiency cores over most of the range. 14th gen (Raptor Lake Refresh) largely repeated the exercise. Arrow Lake (Intel 200) then pivoted to a tile-based design on LGA1851, dropping Hyperthreading from performance cores in exchange for improved efficiency, with Intel Arc graphics now included on-die.


## AMD

- AMD 9000 uses the same AM5 socket as AMD 7000 and is also DDR5-only, though DDR5 pricing has fallen significantly since the 7000 launch. It offers genuinely improved IPC, making it a worthwhile upgrade for existing AM5 owners and a solid choice for new builds.
...
- AMD X3D significantly boosts gaming performance through stacked cache. The 7800X3D and 9800X3D are among the best gaming CPUs at their respective price points, though productivity performance can trail non-X3D alternatives in heavily threaded workloads.
...
| 14100   | 4+0 | 8 | {% assign cpu = site.data.cpus['14100'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14100F  | 4+0 | 8 | {% assign cpu = site.data.cpus['14100F'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14400   | 6+4 | 16 | {% assign cpu = site.data.cpus['14400'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14400F  | 6+4 | 16 | {% assign cpu = site.data.cpus['14400F'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14500   | 6+8 | 20 | {% assign cpu = site.data.cpus['14500'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14600K  | 6+8 | 20 | {% assign cpu = site.data.cpus['14600K'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14600KF | 6+8 | 20 | {% assign cpu = site.data.cpus['14600KF'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14700K  | 8+12 | 28 | {% assign cpu = site.data.cpus['14700K'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14700KF | 8+12 | 28 | {% assign cpu = site.data.cpus['14700KF'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14900K  | 8+16 | 32 | {% assign cpu = site.data.cpus['14900K'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14900KF | 8+16 | 32 | {% assign cpu = site.data.cpus['14900KF'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 14900KS | 8+16 | 32 | {% assign cpu = site.data.cpus['14900KS'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 285K    | 8+16 | 24 | {% assign cpu = site.data.cpus['285K'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 265K    | 8+12 | 20 | {% assign cpu = site.data.cpus['265K'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 265KF   | 8+12 | 20 | {% assign cpu = site.data.cpus['265KF'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 245K    | 6+8 | 14 | {% assign cpu = site.data.cpus['245K'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 245KF   | 6+8 | 14 | {% assign cpu = site.data.cpus['245KF'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
...
| 9600    | 6 | 12 | {% assign cpu = site.data.cpus['9600'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 9600X   | 6 | 12 | {% assign cpu = site.data.cpus['9600X'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 9700X   | 8 | 16 | {% assign cpu = site.data.cpus['9700X'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 9800X3D | 8 | 16 | {% assign cpu = site.data.cpus['9800X3D'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 9900    | 12 | 24 | {% assign cpu = site.data.cpus['9900'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 9900X   | 12 | 24 | {% assign cpu = site.data.cpus['9900X'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
| 9950X   | 16 | 32 | {% assign cpu = site.data.cpus['9950X'] %}{{ cpu.tdp }} | {{ cpu.thread }} | {{ cpu.score }} | {{ cpu.price }} | {{ cpu.rating }} |
